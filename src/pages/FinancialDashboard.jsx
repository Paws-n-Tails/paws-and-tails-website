...
const ChevronDown = ({ className }) => (<svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg>);
const ChevronUp = ({ className }) => (<svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 15l7-7 7 7"></path></svg>);
...
export default function FinancialDashboard({ leads, appointments }) {
  ...
  const [showExpenses, setShowExpenses] = useState(true); // <-- TOGGLE STATE

  const [isExpenseModalOpen, setIsExpenseModalOpen] = useState(false);
  const [isSaving, setIsSaving] = useState(false);
  const [isEmailing, setIsEmailing] = useState(false); 
  const [receiptFile, setReceiptFile] = useState(null);
  const [expenseForm, setExpenseForm] = useState({ date: new Date().toISOString().split('T')[0], amount: '', category: 'Dog Food', description: '' });
  ...
  const allTimeDogTotals = leads.map(lead => {
    const totalSpent = appointments.filter(a => a.lead_id === lead.id).reduce((sum, a) => sum + (Number(a.price) || 0), 0);
  ...
  const generateChartData = () => {
    ...
    for (let i = 12; i >= -2; i--) {
      ...
      data.push({ label: targetDate.toLocaleDateString('en-US', { month: 'short' }) + " '" + targetDate.getFullYear().toString().slice(2), revenue: monthlyRevenue, isCurrent: i === 0, isFuture: i < 0 });
    }
    return data;
  };
    ...
    <div className="flex flex-col h-full bg-gray-50 overflow-hidden">
      <header className="bg-white shadow-sm py-4 px-8 flex justify-between items-center z-20 flex-shrink-0 sticky top-0">
        ...
        <div className="flex items-center gap-3">
          <select value={filterMonth} onChange={(e) => setFilterMonth(e.target.value)} className="border-2 border-gray-200 rounded-xl px-4 py-2 font-bold text-[#104b57] bg-white"><option value="all">All Time</option>{monthOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}</select>
          <button onClick={() => setIsExpenseModalOpen(true)} className="bg-[#d65a47] text-white px-5 py-2.5 rounded-xl text-sm font-black shadow-md hover:bg-[#c44a38] transition-colors flex items-center gap-2"><ReceiptIcon className="w-4 h-4" /> Log Expense</button>
        </div>
      </header>
      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-8 flex flex-col gap-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 flex items-center gap-4"><div className="w-14 h-14 rounded-full bg-green-50 text-green-500 flex items-center justify-center"><DollarIcon className="w-8 h-8" /></div><div><p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Total Revenue</p><h2 className="text-3xl font-black text-[#3a302a]">${totalRevenue.toFixed(2)}</h2></div></div>
          <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 flex items-center gap-4"><div className="w-14 h-14 rounded-full bg-red-50 text-red-500 flex items-center justify-center"><ReceiptIcon className="w-8 h-8" /></div><div><p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Total Expenses</p><h2 className="text-3xl font-black text-[#3a302a]">${totalExpensesAmount.toFixed(2)}</h2></div></div>
          <div className="bg-[#104b57] p-6 rounded-[2rem] shadow-lg border border-[#0c3942] flex items-center gap-4 text-white"><div className="w-14 h-14 rounded-full bg-white/10 flex items-center justify-center"><ChartIcon className="w-8 h-8" /></div><div><p className="text-xs font-bold text-white/70 uppercase tracking-widest">Net Profit</p><h2 className="text-3xl font-black">${netProfit.toFixed(2)}</h2></div></div>
        </div>
            ...
            {chartData.map((data, index) => {
              ...
              return (
                <div key={index} className="flex-1 flex flex-col items-center justify-end h-full relative group">
                  <div className="absolute -top-8 bg-black text-white text-[10px] font-bold py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity z-20">${data.revenue.toFixed(0)}</div>
                  <div className={`w-full rounded-t-md ${data.isCurrent ? 'bg-[#d65a47]' : data.isFuture ? 'bg-[#a3b1c6]' : 'bg-[#104b57]'}`} style={{ height: `${h}%` }}></div>
                  <div className="absolute -bottom-8 text-[9px] font-bold text-gray-400 uppercase text-center w-full whitespace-nowrap">{data.label}</div>
                </div>
              );
            })}
          </div>
        ...
        <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 w-full">
          <div className="flex justify-between items-center mb-10">
            ...
            <select value={topDogsLimit} onChange={(e) => setTopDogsLimit(e.target.value)} className="text-xs font-bold border border-gray-200 rounded-lg p-1.5 focus:outline-none bg-white">
              <option value={5}>Top 5</option><option value={10}>Top 10</option><option value={25}>Top 25</option><option value={50}>Top 50</option><option value={100}>Top 100</option><option value="all">All</option>
            </select>
          </div>
          <div className="flex flex-col gap-6">
            {topDogsDisplay.map((dog, idx) => (
              <div key={dog.id} className="flex items-center gap-4">
                ...
                <div className="w-12 h-12 rounded-full border-2 border-gray-100 overflow-hidden bg-gray-50 flex-shrink-0">{dog.pet_photo ? <img src={dog.pet_photo} className="w-full h-full object-cover" /> : <PawIcon className="w-6 h-6 m-3 text-gray-200"/>}</div>
                <div className="w-32 truncate font-black text-[#3a302a] text-sm">{dog.pet_name}</div>
                <div className="flex-1 h-8 bg-gray-50 rounded-full border border-gray-100 overflow-hidden relative"><div className="bg-gradient-to-r from-[#104b57] to-[#258296] h-full transition-all duration-1000 ease-out rounded-full" style={{ width: `${(dog.totalSpent / maxDogRevenue) * 100}%` }}></div></div>
        ...
        {/* EXPENSE LOG (WITH SHOW/HIDE TOGGLE) */}
        <div className="bg-white rounded-[2rem] shadow-sm border border-gray-200 overflow-hidden mb-10 flex flex-col">
          ...
          >
            <h3 className="font-black text-[#3a302a] text-lg">Expense Log</h3>
            <div className="flex items-center gap-2 text-gray-400 font-bold text-xs uppercase tracking-widest">
              {showExpenses ? "Hide Table" : `Show Table (${filteredExpenses.length})`}
              ...
              <table className="w-full text-left">
                ...
                <tbody>
                  {filteredExpenses.length === 0 ? (
                    <tr><td colSpan="4" className="p-10 text-center text-gray-400 font-bold">No expenses found for this period.</td></tr>
                  ) : (
